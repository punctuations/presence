...
export default async function handler(
  ...
  return new Promise(async (resolve) => {
    ...
    axios
      .get(`https://api.producthunt.com/v1/users/${id}`, {
        headers: {
      ...
      .then(async (r: AxiosResponse) => {
        ...
        res.send(
          query.type?.toLowerCase() === "base64"
            ? {
                data: await base(await ProductHuntUserImage(r.data, query)),
              }
            : query.type?.toLowerCase() === "png"
            ? await convert(await ProductHuntUserImage(r.data, query), 938, 285)
            : await ProductHuntUserImage(r.data, query)
        );
