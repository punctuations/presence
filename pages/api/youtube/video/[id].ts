...
export default async function handler(
  ...
  return new Promise(async (resolve) => {
    ...
    axios
      .get(
        `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&id=${id}&key=${process.env.YOUTUBE_TOKEN}`
      )
      .then(async (r: AxiosResponse) => {
        ...
        res.send(
          query.type?.toLowerCase() === "base64"
            ...
            ? await convert(await YoutubeVideoImage(r.data, query), 938, 285)
